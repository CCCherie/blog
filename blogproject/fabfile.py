...
def deploy(c):
    ...
    project_root_path = '~/apps2/blog/blogproject'
    ...
    with c.cd(project_root_path):
        ...
        c.run('~/.local/bin/pipenv run python3 manage.py migrate')
        c.run('~/.local/bin/pipenv run python3 manage.py collectstatic --noinput')
